...
FuncNode* GetFuncByName(char* pstrName)
{
	...
	for (int iCurrFuncIndex = 1; iCurrFuncIndex <= g_FuncTable.iNodeCount; ++iCurrFuncIndex)
	{
		...
		if (pCurrFunc && _stricmp(pCurrFunc->pstrName, pstrName) == 0)
			return pCurrFunc;
	}
...
int AddFunc(char* pstrName, int iIsHostAPI)
{
	...
	// Add the function to the list and get its index, but add one since the zero index is reserved for the global scope
	...
	//如果是Main函数，设置flag和索引标记
	if (_stricmp(pstrName, MAIN_FUNC_NAME) == 0)
	{
