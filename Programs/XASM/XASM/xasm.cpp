...
/*
Assembles XVM Assembly scripts to .XSE executables for use with the XtremeScriptVirtual Machine (XVM).
This updated version 0.8 also preserves script-defined function names for late binding and stores thread priority.
...
// An instruction lookup 指令查找表
typedef struct _InstrLookup                     
{
	char pstrMnemonic[MAX_INSTR_MNEMONIC_SIZE];  // Mnemonic string 助记符
	int iOpcode;                                // Opcode 操作码
	int iOpCount;                               // Number of operands
	OpTypes* OpList;                           // Pointer to operand list 指向操作数列表的指针
}InstrLookup;

// ---- Assembled Instruction Stream ------------------------------------------------------

// An assembled operand
typedef struct _Op                              
{
	int iType;                                  // Type
	union                                       // The value
...
//A label table node 标签表的节点
typedef struct _LabelNode                       
{
	int iIndex;									// Index
	char pstrIdent[MAX_IDENT_SIZE];          // Identifier
	int iTargetIndex;                           // Index of the target instruction 目标指令 的索引
	int iFuncIndex;                             // Function in which the label resides 标签所属的函数
}LabelNode;
...
/// <summary>
/// Strips the comments from a given line of source code, ignoring comment symbols found nside strings. 
///The new string is shortended to the index of the comment symbol character.
/// 消除注释
/// </summary>
...
/// <summary>
/// Returns a nonzero if the given string is composed entirely of valid identifier characters and begins with a letter or underscore. 
...
/// <summary>
/// Returns a nonzero if the given string is composed entire of integer characters, or zero otherwise.
/// </summary>
/// <param name="pstrString"></param>
/// <returns></returns>
int IsStringInteger(char* pstrString)
...
/// <summary>
/// Returns a nonzero if the given string is composed entire of float characters, or zero otherwise.
/// </summary>
/// <param name="pstrString"></param>
/// <returns></returns>
int IsStringFloat(char* pstrString)
...
/// </summary>
/// <param name="pstrMnemonic"></param>
/// <param name="iOpcode"></param>
/// <param name="iOpCount"></param>
/// <returns></returns>
int AddInstrLookup(char* pstrMnemonic, int iOpcode, int iOpCount)
{
...
/// <summary>
/// Extracts and returns the next token from the character stream. 
/// Also makes a copy of the current lexeme for use with GetCurrLexeme ().
/// 获取下一个属性符，暴力扫描方式，未使用状态机方法
/// </summary>
/// <returns></returns>
Token GetNextToken()
{
	//! Lexeme Extraction 单词提取

	// Move the first index (Index0) past the end of the last token, which is marked by the second index (Index1).
	//将第一个索引移到最后一个属性符的尾部，最后一个属性符尾部是用第二个索引标识的
	...
	// If we just ended a string, tell the lexer to stop lexing strings and return to the normal state
	//如果结束对字符串的分析。就要停止对字符串的词法分析
	...
	if (g_Lexer.iCurrLexState != LEX_STATE_IN_STRING)
	{
		...
		while (TRUE)
		{
			// If the current character is not whitespace, exit the loop because the lexeme is starting.
			//如果当前字符不是空格
	...
	// Bring the second index (Index1) to the lexeme's starting character, which is marked by the first index (Index0)
	//让通过第一个索引标识的第二个索引指向单词开始的字符
...
/// <summary>
/// Skips to the next line in the character stream. 
///Returns FALSE the end of the source code has been reached, TRUE otherwise.
/// </summary>
/// <returns></returns>
int SkipToNextLine()
{
...
/// <param name="iLocalDataSize"></param>
void SetFuncInfo(char* pstrName, int iParamCount, int iLocalDataSize)
{
	...
	while (TRUE)
	{
		...
		case TOKEN_TYPE_SETPRIORITY:
			...
			case TOKEN_TYPE_IDENT:
				...
				if (_stricmp(g_Lexer.pstrCurrLexeme, PRIORITY_LOW_KEYWORD) == 0)
					g_ScriptHeader.iPriorityType = PRIORITY_LOW;
				else if (_stricmp(g_Lexer.pstrCurrLexeme, PRIORITY_MED_KEYWORD) == 0)
					g_ScriptHeader.iPriorityType = PRIORITY_MED;
				else if (_stricmp(g_Lexer.pstrCurrLexeme, PRIORITY_HIGH_KEYWORD) == 0)
...
void BuildXSE()
{
	...
	//! Write the instruction stream 写指令流
